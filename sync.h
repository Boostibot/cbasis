...
typedef union Ticket_Lock {
    struct {
        CHAN_ATOMIC(uint32_t) requested;
        CHAN_ATOMIC(uint32_t) completed;
    };
...
CHANAPI void ticket_lock(Ticket_Lock* lock, Sync_Wait wait)
{
    ...
    for(;;) {
        uint32_t curr_completed = atomic_load(&lock->completed);
...
static void sync_barrier(Barrier* barrier, Sync_Wait wait, uint32_t count)
{
    ...
    uint32_t t = (uint32_t) (h_t >> 32);

    while(true) {
        if(h >= t) {
            if(h+count == t+1)
                atomic_store(&barrier->head, h+count);
...
static void sync_barrier_lift(Barrier* barrier, Sync_Wait wait, uint32_t count)
{
    ...
    if(h < t) {
        uint32_t curr_h = h;
        while(true) {
            if(atomic_compare_exchange_weak(&barrier->head, &curr_h, h+count))
                break;
            
            curr_h = atomic_load(&barrier->head);
            if(curr_h >= h+count)
                break;
        }
    }
...
// This implementation guarantees that the waiting thread will get woken up exactly once. 
